...
import { adminClient } from '../../src/lib/supabaseServer' // use '@/src/...' if your alias works
        ...
        {(posts ?? []).map(p => (
          <article key={p.slug} className="card">
            ...
            <p className="text-slate-700 mt-2">
              {strip((p.html as string) || '').slice(0, 180)}…
            </p>
            <div className="mt-3">
              <Link href={`/blog/${p.slug}`} className="btn">Read more</Link>
            </div>
          </article>
        ))}
      </div>
